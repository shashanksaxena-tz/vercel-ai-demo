...
export const Comment = ({ element, children, onAction }: ComponentRenderProps) => {
  const {
    id,
    author,
    avatar,
  ...
  return (
    ...
    >
      {avatar ? (
        <img src={avatar as string} alt={author as string} className="w-8 h-8 rounded-full flex-shrink-0" />
      ) : (
      ...
      <div className="flex-1">
        <div className="bg-muted rounded-lg px-3 py-2">
          <p className="font-semibold text-sm">{author as string}</p>
