...
export const AreaChart = ({ element }: ComponentRenderProps) => {
  const {
    data,
    title,
    series,
  ...
  const getPoints = (valueKey: string = 'value') => {
    return chartData.map((item, idx) => {
      ...
      const y = 180 - (item[valueKey] / maxValue) * 160;
      return { x, y, value: item[valueKey], label: item.label };
    });
  };

  const getAreaPath = (points: Array<{ x: number; y: number }>) => {
    const linePoints = points.map((p) => `${p.x},${p.y}`).join(' L ');
    return `M ${points[0].x} 180 L ${linePoints} L ${points[points.length - 1].x} 180 Z`;
  };

  const renderSeries = (valueKey: string, index: number) => {
    const points = getPoints(valueKey);
    const color = getColor(index);
    const gradientId = `gradient-${valueKey}-${index}`;
        ...
        <svg className="w-full h-full" viewBox="0 0 400 200" preserveAspectRatio="none">
          {showGrid && (
            <g stroke="#e5e7eb" strokeWidth="1" strokeDasharray="4">
              {[0, 1, 2, 3, 4].map((i) => (
                <line key={`h${i}`} x1="10" y1={20 + i * 40} x2="390" y2={20 + i * 40} />
              ))}
            </g>
          )}
          {seriesNames
